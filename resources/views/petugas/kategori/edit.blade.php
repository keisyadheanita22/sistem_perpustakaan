...
<nav style="background-color: #2D3A1E; height: 56px; padding: 0 32px; display: flex; align-items: center; justify-content: space-between; position: sticky; top: 0; z-index: 50; border-bottom: 2px solid #D4A017;">
    ...
    <div style="display: flex; min-height: calc(100vh - 56px);">
        ...
        {{-- ============================================= --}}
        {{-- KONTEN UTAMA - card ditengah secara vertikal  --}}
        {{-- ============================================= --}}
        <main style="flex: 1; display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 32px;">
            ...
            <div style="width: 100%; max-width: 480px; margin-bottom: 20px;">
                <h1 style="font-size: 24px; font-weight: 700; color: #2D3A1E; margin: 0;">Edit Kategori</h1>
                <p style="font-size: 13px; color: #8A7E6E; margin: 6px 0 0 0;">Perbarui nama kategori yang sudah ada</p>
            </div>
            ...
            <div style="width: 100%; max-width: 480px; background: #F9F9F9; border-radius: 16px; border: 1px solid #D4A017; box-shadow: 0 8px 24px rgba(0,0,0,0.10); overflow: hidden;">
                ...
                {{-- Header card: ikon pensil + deskripsi --}}
                <div style="display: flex; align-items: center; gap: 14px; padding: 18px 24px; border-bottom: 1px solid #E8E2D4; background: #FFFDF8;">
                    {{-- Ikon pensil/edit sebagai representasi halaman edit, aksen kuning emas --}}
                    <div style="width: 42px; height: 42px; border-radius: 10px; background-color: #FEF3C7; display: flex; align-items: center; justify-content: center; flex-shrink: 0;">
                        <svg xmlns="http://www.w3.org/2000/svg" style="width: 20px; height: 20px;" fill="none" viewBox="0 0 24 24" stroke="#92400E">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                        </svg>
                    </div>
                    <div>
                        <p style="font-size: 14px; font-weight: 700; color: #2D3A1E; margin: 0;">Ubah Kategori</p>
                        <p style="font-size: 12px; color: #8A7E6E; margin: 4px 0 0 0;">Nama lama: <span style="font-weight: 600; color: #2D3A1E;">{{ $kategori->nama_kategori }}</span></p>
                    </div>
                </div>

                {{-- FORM EDIT KATEGORI: kirim ke KategoriController@update via PUT --}}
                <form method="POST" action="{{ route('kategori.update', $kategori->id) }}" style="padding: 24px;">
                    @csrf
                    @method('PUT')

                    {{-- Input nama kategori, nilai awal diisi dari data yang sudah ada --}}
                    <div style="margin-bottom: 8px;">
                        <label style="display: block; font-size: 13px; font-weight: 600; color: #2D3A1E; margin-bottom: 8px;">
                            Nama Kategori
                        </label>
                        {{-- old() menjaga nilai input tetap ada jika validasi gagal, fallback ke data kategori --}}
                        <input type="text"
                               name="nama_kategori"
