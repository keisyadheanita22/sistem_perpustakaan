...
<nav style="background:#2D3A1E; display:flex; justify-content:space-between; align-items:center; padding:0 32px; height:60px; border-bottom: 2px solid #D4A017; position: sticky; top: 0; z-index: 100;">
    ...
    <span style="color:#F5F0E8; font-size:14px; background: rgba(255,255,255,0.1); padding: 6px 14px; border-radius: 20px;">
        {{ Auth::user()->name }} <span style="opacity: 0.7; margin-left: 4px;">(Kepala)</span>
    </span>
</nav>
...
<div style="display:flex;">
    ...
    <main style="flex:1; padding:40px;">
        ...
        {{-- NOTIFIKASI FLASH --}}
        @if(session('success'))
            <div id="flash-msg" style="background:#D4A017; color:#2D3A1E; padding:12px 20px; border-radius:10px; margin-bottom:20px; font-weight:bold; font-size:14px; border:1px solid #2D3A1E; animation: fadeIn 0.5s;">
                ✅ {{ session('success') }}
            </div>
        ...
        <div style="display:grid; grid-template-columns: 320px 1fr; gap:32px; align-items: start;">
            ...
            <div style="background:#FFFFFF; border-radius:20px; padding:32px; text-align:center; box-shadow:0 10px 25px rgba(0,0,0,0.04); border: 1px solid #E8E2D4;">
                ...
                <p style="font-size:14px; color:#8A7E6E; margin-top: 6px; margin-bottom: 24px; font-weight: 500;">Kepala Perpustakaan</p>

                <form method="POST" action="{{ route('kepala.profil.foto') }}" enctype="multipart/form-data" style="border-top: 1px solid #F0EBE0; padding-top: 24px;">
                    @csrf
                    ...
                    <input type="file" name="foto" required style="font-size: 12px; width: 100%; margin-bottom: 20px;">
            ...
            <div style="display:flex; flex-direction:column; gap:28px;">
                ...
                <div style="background:#FFFFFF; padding:32px; border-radius:20px; box-shadow:0 10px 25px rgba(0,0,0,0.04); border: 1px solid #E8E2D4;">
                    ...
                    <form method="POST" action="{{ route('kepala.profil.update') }}" style="margin-top: 20px;">
                        @csrf
                        @method('PUT')
